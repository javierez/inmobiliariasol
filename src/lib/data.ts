import type { HeroMediaItem } from "./hero-media";
...
export type HeroProps = {
  title: string;
  subtitle: string;
  /** Ordered hero background slides. Legacy configs only have the two fields below. */
...
export type AboutProps = {
  ...
  extendedServices?: Array<{
    title: string;
    description: string;
    // Optional, like ExtendedServicesGrid has always treated it: the card
    // leads with its image and simply omits the glyph when there is none.
    // Required here, it blocked account 122 from publishing at all — three
    // services saved without an icon, and every build failed on the type.
    icon?: string;
    image?: string;
    bullets?: string[];
    ctaLabel?: string;
...
/** Which of the two testimonial card designs the account picked in the CRM. */
export type TestimonialCardStyle = "avatar" | "photo";

export type TestimonialProps = {
  title: string;
  subtitle: string;
  itemsPerPage: number;
  /** Off (or absent) hides the whole section from the homepage. */
  enabled?: boolean;
  /** Absent means the classic avatar card. */
...
export type FooterProps = {
  ...
  legalBadges?: Array<{
    ...
    alt: string;
    href?: string;
    // Tira horizontal con varios logos (p. ej. Kit Digital): ocupa el ancho.
    wide?: boolean;
  }>;
};
