...
export const properties = pgTable("properties", {
  ...
  // Property Condition
  // El estado del inmueble vive en `conservationStatus` (única fuente de verdad).
  // Las columnas `brand_new` / `needs_renovation` duplicaban esa información y se
  // han eliminado.
  ...
  // Acondicionamiento del local. Tri-estado a propósito — NULL es "no consta",
  // y SIN DEFAULT (migración 0350 del CRM) para que una ficha nueva no nazca
  // afirmando "sin acondicionar". Mantener sin `.default(false)`: si este espejo
  // del esquema lo reintroduce, un `drizzle-kit push` desde aquí volvería a
  // poner el DEFAULT en la base compartida y desharía esa migración.
  isFittedOut: boolean("is_fitted_out"),
...
export const listings = pgTable("listings", {
  ...
  // Instante del cierre comercial (Vendido / Alquilado / Descartado). Ancla de
  // la ventana "un vendido sigue visible N días" — ver
  // `visibleStatusCondition` en queries/filters.ts. Deliberadamente NO se usa
  // `updatedAt`: cualquier edición lo reinicia. NULL = nunca cerrado o
  // reabierto. Migración 0305 del CRM.
...
export const documents = pgTable("documents", {
  ...
  // La promoción de obra nueva de la que cuelga el PDF (0340).
  promotionId: bigint("promotion_id", { mode: "bigint" }), // FK → promotions.promotion_id (nullable)
...
// Blog posts table — articles for the public agency website (/blog).
// Authored from the CRM website editor; the site only renders rows whose
// status is 'published' and whose published_at has already passed.
export const blogPosts = pgTable("blog_posts", {
  postId: bigserial("post_id", { mode: "bigint" }).primaryKey(),
  accountId: bigint("account_id", { mode: "bigint" }).notNull(), // FK → accounts.account_id
  slug: varchar("slug", { length: 255 }).notNull(), // URL segment: /blog/<slug>
  title: varchar("title", { length: 500 }).notNull(),
  excerpt: text("excerpt"), // card copy + fallback meta description
  content: text("content").notNull().default(""), // markdown body
  coverImage: varchar("cover_image", { length: 1024 }), // S3 URL — hero + card image
  coverAlt: varchar("cover_alt", { length: 500 }),
  authorName: varchar("author_name", { length: 255 }),
  authorRole: varchar("author_role", { length: 255 }),
  authorImage: varchar("author_image", { length: 1024 }),
  category: varchar("category", { length: 120 }),
  tags: text("tags"), // JSON string[] — same JSON-in-text convention as website_config
  status: varchar("status", { length: 20 }).notNull().default("draft"), // 'draft' | 'published'
  featured: boolean("featured").notNull().default(false), // pinned as the big hero post on /blog
  // Manual order on /blog, ascending. 0 means "never dragged", so an account
  // that has not touched the order keeps falling back to newest-first — and a
  // brand-new article lands at the top instead of behind whatever was sorted.
  sortOrder: integer("sort_order").notNull().default(0),
  readMinutes: smallint("read_minutes"), // null → estimated from content length
  // Lecturas del artículo público. Lo suma el navegador al abrirlo
  // (`registerBlogView`), no el render: la página va con ISR y contar en
  // servidor contaría regeneraciones, no visitas. Orientativo: no distingue
  // visitantes únicos.
  viewCount: integer("view_count").notNull().default(0),
  ...
  uniqueIndex("uq_blog_posts_account_slug").on(table.accountId, table.slug),
  index("blog_posts_account_status_published_idx").on(
    table.accountId,
    table.status,
    table.publishedAt,
  ),
  index("blog_posts_account_sort_idx").on(
    table.accountId,
    table.sortOrder,
    table.publishedAt,
  ),
...
export const websiteProperties = pgTable("website_config", {
  ...
  // Nombre del proyecto de Vercel que sirve esta web. 🚨 Es la fuente de verdad
  // del destino del despliegue: NO derivar de `accounts.name`, que la agencia
  // edita. null = nunca desplegada → se deriva y se sella (ver resolveProjectName).
  vercelProject: varchar("vercel_project", { length: 100 }),
  ...
  blogProps: text("blog_props"), // JSON BlogProps — /blog page settings (hero, titles, menu visibility). null → blog hidden (default for all accounts)
